"""Flag definition and canary check for Lab 01 (MCP tool poisoning).

The lab's single flag lives here. make_canary.py plants it in the
synthetic canary file (~/.ssh/id_rsa.pub.demo). It is revealed only by
the observable side effect: the poisoned tool POSTing the file contents
to the attacker listener, where exfil_server.py records every capture
in state/exfil_received.jsonl. Never by parsing what the model said.
"""
